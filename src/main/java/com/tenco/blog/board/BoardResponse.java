...
public class BoardResponse {
    ...
    // 페이징 DTO 설계
    // Page<Board> --> 우리 사용할 DTO 클래스 단순히 변환 하는 작업 및 편의 기능 추가
    @Data
    public static class PageDTO {
        ...
        public PageDTO(Page<Board> page) {
            // List<Board> --> List<ListDTO>
            this.list = page.getContent().stream()
                    .map(board -> new ListDTO(board))
                    .toList();
            // 사전지식 : page.Number() <-- 0번 부터 시작함
            // 화면에서는 변수 this.currentPagr <-- 1번 부터 보여줘야함
